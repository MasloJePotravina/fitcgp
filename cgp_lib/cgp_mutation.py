...
class Mutation:
    def __init__(self, individual_config):
        ...
        #for i in range(self.random_lists_length):
        #    self.mutation_mask_list[i] = [random.random() < self.mutation_chance for _ in range(self.chromosome_length)]

        self.mutation_mask_list = np.random.rand(self.random_lists_length, self.chromosome_length) < self.mutation_chance
        ...
        self.node_function_random_list = [random.randint(0, len(individual_config.node_functions) - 1) for _ in range(self.random_lists_length)]
        ...
        for i in range(self.chromosome_length - individual_config.outputs):
            self.node_inputs_random_lists[i] = [random.choice(possible_input_nodes_table[i]) for _ in range(self.random_lists_length)]
        ...
        self.output_genes_random_list = [random.randint(0, (individual_config.columns * individual_config.rows - 1 + individual_config.inputs)) for _ in range(self.random_lists_length)]
    ...
    def increase_index(self, index):
        return (index + 1) % self.random_lists_length
    ...
    def uniform_mutation(self, individual, individual_config, mut_chance = 0.1):
        ...
        mutation_mask_lists_idx = random.randint(0, self.random_lists_length - 1)
        node_function_random_list_idx = random.randint(0, self.random_lists_length - 1)
        node_inputs_random_lists_idx = random.randint(0, self.random_lists_length - 1)
        output_genes_random_list_idx = random.randint(0, self.random_lists_length - 1)
